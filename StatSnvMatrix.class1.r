...
remove<-NULL
Sum<-NULL
pdf(width=18, height=15,file="MetaPlot.Class1.pdf")
for(u in unique(index[,3])){
 ...
 for(i in 1:nrow(FULL[[1]])){
  ...
  if(nrow(tres) < 3) next
  tres[,4]<-ifelse(tres[,4]==0, tres[,7] * 0.001, tres[,4])
...
write.table(Sum, "StatSnvMatrix.class1.txt", row.names=TRUE, col.names=FALSE, quote=FALSE, sep="\t")