...
OUTPUT_DIR=/share/icgc_pan/WORK/hasegawa/NeoAntimon/Output.Snv.2.new
...
do
  ...
  if [ -e ${OUTPUT_DIR}/${sample} ]; then
     ...
     echo "./../../Exe_snv_class2.sh ${HOME} ${main} ${sample} ${correspo} ${annotation} ${hla} ${rna} ${rna_bam} ${genomon} ${ascat} ${purity}"
     qsub -l ljob ./../../Exe_snv_class2.sh ${HOME} ${main} ${sample} ${correspo} ${annotation} ${hla} ${rna} ${rna_bam} ${genomon} ${ascat} ${purity}
     cd ${OUTPUT_DIR}
     COUNT=`expr $COUNT + 1`
  fi
  echo "---"
