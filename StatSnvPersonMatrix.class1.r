...
#for(u in unique(index[,3])){
 ...
 print(u)
 pdf(width=18,height=15,file=paste("StatSnvPersonMatrix/HLA.Boxplot.Class1.FULL",u,fpkm,"pdf",sep="."))
 FULL<-NULL
 FULL_sep<-NULL
 HIT<-NULL
 tag<-NULL
 ...
 for(j in 1:length(FULL)){
  ...
  boxplot(Ftmp, xlim=c(0.5,ncol(Ftmp)+0.5), ylim=c(min(Ftmp)-1,max(Ftmp)*1.1), main="Fix HLAs",
  ...
  plot(1:ncol(Ftmp), sapply(1:ncol(Ftmp), function(x) Ftmp[x,x]),pch=4, 
   xlim=c(0.5,ncol(Ftmp)+0.5),ylim=c(min(Ftmp)-1,max(Ftmp)*1.1),xlab="", ylab="")
  ...
  for(k in 1:nrow(FULL[[j]])){
   ...
   eachsubject<-rbind(eachsubject, c(tag[j], strsplit(rownames(FULL[[j]])[k]," ")[[1]][1], 
    lm(FULL[[j]][k, ] ~ HIT[[j]][k, ])$coefficients[2]))
  }
 ...
 for(j in 1:length(FULL_sep)){
  ...
  count<-1
  ylim_<-c(max(0, min(val) * 0.9), min(1, max(val)* 1.1))
  plot(0:6, sapply(0:6, function(z) median(FULL_sep[[j]][1,grep(z, HIT[[j]][1,])])),type="b",xlim=c(0,6),ylim=ylim_,col=count,pch=count)
  ...
  for(k in 1:nrow(FULL_sep[[j]])){
   ...
   eachsubject<-rbind(eachsubject, c(paste("Sep", tag[j]), strsplit(rownames(FULL[[j]])[k]," ")[[1]][1], 
    lm(FULL[[j]][k, ] ~ HIT[[j]][k, ])$coefficients[2]))
  }
  hist(dist, breaks="Scott", main=paste("pval=", t.test(dist)$p.value))
 }
...
write.table(eachsubject,paste("StatSnvPersonMatrix/HLA.Boxplot.Class1.FULL.lm",u,fpkm,"txt",sep="."), 
	row.names=FALSE, col.names=FALSE, quote=FALSE, sep="\t")
write.table(eachposition,paste("StatSnvPersonMatrix/HLA.Boxplot.Class1.FULL.position",u,fpkm,"txt",sep="."), 
        row.names=FALSE, col.names=FALSE, quote=FALSE, sep="\t")
print("Remove")
print(remove)
dev.off()
