...
OUTPUT_DIR=/share/icgc_pan/WORK/hasegawa/NeoAntimon/Output.Indel.2
...
do
  ...
  if [ -e ${OUTPUT_DIR}/${sample} ]; then
     ...
     echo "./../../Exe_indel_class2.sh ${HOME} ${main} ${sample} ${correspo} ${annotation} ${hla} ${rna} ${rna_bam} ${genomon} ${ascat} ${purity}"
     qsub ./../../Exe_indel_class2.sh ${HOME} ${main} ${sample} ${correspo} ${annotation} ${hla} ${rna} ${rna_bam} ${genomon} ${ascat} ${purity}
     cd ${OUTPUT_DIR}
     COUNT=`expr $COUNT + 1`
  fi
  echo "---"
