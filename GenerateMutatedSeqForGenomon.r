...
index<-strsplit(scan(commandArgs(TRUE)[2], "character", sep="\n", nlines=1), "\t")[[1]]
...
for(i in 1:length(data)){
   ...
   MP<-0#strsplit(strsplit(f[grep("MP=",f)], "MP=")[[1]][2],";")[[1]][1]
   GP<-0#strsplit(strsplit(f[grep("GP=",f)], "GP=")[[1]][2],";")[[1]][1]
   DP<-as.numeric(f[grep("depth_normal", index)]) + as.numeric(f[grep("depth_tumor", index)])
   TDP<-as.numeric(f[grep("depth_tumor", index)])
   ...
   nm_ids<-strsplit(f[grep("NM_", f[1:10])], ":|,")
   hit<-as.numeric(sapply(nm_ids, function(x) grep("NM_", x)))
   Pass<-FALSE
   #Calculate All NM_IDs in Each Mutation
   for(h in hit){
      #For example, nm_ids[[1]]="SKI"       "NM_003036" "exon1"     "c.T962G"   "p.V321G"...
      g_name<-nm_ids[[1]][h - 1]
      nm_id<-nm_ids[[1]][h]
      ...
      #Obtain refFLAT Data
      s_variants<-grep(nm_id, list_nm_cut)
	 ...
	 stop_loop<-FALSE
	    ...
	    peptide_start<-ceiling(m_point_2/3.0) - pep_len
            ...
            refFasta<-rbind(refFasta,
               c(paste(random, gsub("\"","", g_name), sep="_"), chr, nm_ids[[1]][h], nm_ids[[1]][h+2], m_ref, m_alt, MP, GP,
	    ...
	    #Remove X and Save Fasta
            ...
            fasta<-c(fasta, sub(":","",paste(">", random, gsub("\"","", g_name), nm_id, m_point, 
	    ...
	    fasta_norm<-c(fasta_norm, sub(":","",paste(">", random, gsub("\"","", g_name), nm_id, m_point, 
               m_ref, m_alt, target_amino_before, target_amino_after,sep=":")))
...
while(i<=nrow(refFasta)){
   ...
   hit<-which((refFasta[i, 1]==refFasta[,1])&(refFasta[i, 11]==refFasta[,11]) &
      (refFasta[i, 14]==refFasta[,14])&(refFasta[i, 15]==refFasta[,15]))
   if(length(hit)==1){
      i<-i+1
      next
   }
