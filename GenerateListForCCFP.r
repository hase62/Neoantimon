...
dir<-"seqware-results/0/ascat/"
files<-list.files(dir)
cnv<-paste(dir,files[grep("copynumber.txt",files)],sep="/")
#ss<-paste(dir,files[grep("samplestatistics",files)],sep="/")
...
flag<-FALSE
if(!is.na(commandArgs(TRUE)[2]) & !is.na(as.numeric(commandArgs(TRUE)[2]))){
 purity<-as.numeric(commandArgs(TRUE)[2])
} else{
 flag<-TRUE
...
#Write List For CCFP
write.table(cnv, paste(commandArgs(TRUE)[1],".cnv.txt",sep=""), 
   row.names=FALSE, col.names=FALSE, quote=FALSE, sep="\t")

#OverWrite peptide.txt
if(length(remove)>0){
   write.table(cbind(data, cnv[1 + match(data[,1], sapply(cnv[-1,1], function(x) strsplit(x, ";")[[1]][2])),c(2,3)]), commandArgs(TRUE)[1], 
      row.names=FALSE, col.names=FALSE, quote=FALSE, sep="\t")
} else {
   if(nrow(data)==1){
         write.table(t(c(data[1,], cnv[-1,c(2,3)])), commandArgs(TRUE)[1], 
      row.names=FALSE, col.names=FALSE, quote=FALSE, sep="\t")
   } else {
      write.table(cbind(data, cnv[1 + match(data[,1], sapply(cnv[-1,1], function(x) strsplit(x, ";")[[1]][2])),c(2,3)]), commandArgs(TRUE)[1], 
         row.names=FALSE, col.names=FALSE, quote=FALSE, sep="\t")   
