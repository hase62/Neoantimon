...
remove<-NULL
pdf(width=18, height=15,file="MetaPlot.Class2.pdf")
for(u in unique(index[,3])){
 ...
 for(i in 1:nrow(FULL[[1]])){
  ...
  hla2<-colnames(meta)<-ifelse(is.na(sp)|sp=="Unclassified",gsub("_","*",colnames(meta)),sp)
  ...
  for(h in 1:length(unique(hla2))){
   ...
   tres<-rbind(tres, c(t.test(mean_a,mean_b)$p.value, length(mean_a), 
    mean(mean_a), sqrt(var(mean_a)), length(mean_b), mean(mean_b), sqrt(var(mean_b))))
   rownames(tres)[nrow(tres)]<-unique(hla2)[h]
  }
  tres<-tres[!is.na(tres[,1]),]
  if(is.null(tres)) next
...
write.table(Sum, "StatSnvMatrix.class2.txt", row.names=TRUE, col.names=FALSE, quote=FALSE, sep="\t")
