...
public class SimulatorWithStemCell extends Simulator{
...
public  SimulatorWithStemCell (){}
	
	public  SimulatorWithStemCell (SimulatorWithStemCell S){
	...
	void growPopulation(){
		...
		for(int i = 0; i < n & i < population.size(); i++){
			...
			if((C.isStem & R.nextDouble() < deathRate) | (!C.isStem & R.nextDouble() < deathRateForNonStem)){
				population.remove(C);
				i--;
			}
		}
