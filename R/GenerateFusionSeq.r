GenerateIndelSeq<-function(input_file, 
  ...
  mateIDs<-sapply(data, function(x) strsplit(strsplit(x, "MATEID=")[[1]][2], "_|;")[[1]][1])
  ...
  list_nm_c  <-scan(paste(hmdir,"/refFlat.c1.txt",    sep=""), "character", sep="\n")
  ...
  pep_len<-as.numeric(commandArgs(TRUE)[3])
  random<-0
  fasta<-NULL
  refFasta<-NULL
  for(uID in uIDs){
        ...
        for(v in s_variants){
          for(mate_whether_exon_intron in c("exon", "intron")){
            ...
            if(whether_exon_intron == "exon" & length(which((exon_start - m_start) * (exon_end - m_start) <= 0)) == 0){
              ...
              print(paste("Annotation is Intronic but Position is not Intron", 
                          paste(exon_start, collapse=":"), 
                          paste(exon_start, collapse=":"), 
                          m_start))
              next
            }
          ...
          for(row2 in 1:nrow(sub_set)){
            ...
            if(flag){
              ...
              if(frame == "In"){
                ...
                peptide_end <- length(peptide) - peptide_end + pep_len + 10
  ...
  write.table(fasta, paste(commandArgs(TRUE)[1],"peptide.svs","fasta",sep="."),
              row.names=FALSE, col.names=FALSE, quote=FALSE, sep="\t")
  write.table(refFasta, paste(commandArgs(TRUE)[1],"peptide.svs","txt",sep="."),
