#' Merge Results from MainSnvClass2.R
#'
#'@param input_dir Directory storing netMHCpan Results (Required).
#'
#'@param input_file_prefix File prefix of netMHCpan Results (Required).
#'If you have "sample_annovar.txt.NO_JOB_ID.HLACLASS1.1.peptide.txt", please set "sample_annovar", "sample_annovar.txt" or "sample_annovar.txt.NO_JOB_ID".
#'
...
#'
#'@export
MainMergeClass2<-function(hmdir = getwd(), input_dir, input_file_prefix, Tumor_RNA_BASED_ON_DNA = TRUE, INDEL = FALSE){
  ...
  colnames(rank_peptide_50)<-tag
   ...
   write.table(min_peptide_50, paste(dir, "/", input_file_prefix, ".CLASS2.IC50min_WTupper500.txt", sep=""), row.names=FALSE, col.names=TRUE, quote=FALSE, sep="\t")
   write.table(rank_peptide, paste(dir, "/", input_file_prefix, ".CLASS2.RANKmin.txt", sep=""), row.names=FALSE, col.names=TRUE, quote=FALSE, sep="\t")
   write.table(rank_peptide_50, paste(dir, "/", input_file_prefix, ".CLASS2.RANKmin_WTupper2.txt", sep=""), row.names=FALSE, col.names=TRUE, quote=FALSE, sep="\t")
}
