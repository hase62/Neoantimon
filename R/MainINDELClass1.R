#'Calculate Neoantigen Candidates on Indels for MHC Class1
#'
...
MainINDELClass1<-function(input_file, hla_file, file_name_in_hla_table = input_file,
  ...
  GetRNAseq_indel(output_peptide_txt_file = output_peptide_txt_file, rnaexp_file = rnaexp_file,
  ...
  if(ifelse(is.na(cnv_file), FALSE, file.exists(cnv_file))){
    ...
    if(file.exists(paste(output_peptide_txt_file,".s_file.txt",sep=""))){
