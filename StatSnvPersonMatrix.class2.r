...
#RemoveHomo
index[,4]<-sapply(index[,4], 
	     function(x) paste(collapse=" ", unique(strsplit(x, " ")[[1]][grep("DRB", strsplit(x, " ")[[1]])])))
index<-index[sapply(index[,4], function(x) length(unique(strsplit(x, " ")[[1]])))==2,]

remove<-NULL
#for(u in unique(index[,3])){
 u<-unique(index[,3])[as.numeric(commandArgs(TRUE)[1])]
 pdf(width=18,height=15,file=paste("StatSnvPersonMatrix/HLA.Boxplot.Class2.FULL",u,fpkm,"pdf",sep="."))
 FULL<-NULL
 FULL_sep<-NULL
 HIT<-NULL
 tag<-NULL
 ...
 for(i in l){
  ...
  for(criteria in c("min_lower50.txt","min_upper500.txt")){
   ...
   #Remove Na from position
   peptide<-peptide[which(peptide[,21]!="NA"),]
   colnames(peptide)<-strsplit(scan(paste(dir, index[i,1], f, sep="/"), 
     "character", sep="\n", nlines=1), "\t")[[1]]

   #Extract FPKM threshold
   if(fpkm!=0) {
     peptide<-peptide[!is.na(as.numeric(peptide[,28])) & as.numeric(peptide[,28]) > fpkm,]
   }
   if(nrow(peptide)==0) next
   for(ic50 in c(50, 200, 500)){
    neoAntigen_on_i<-NULL
    neoAntigen_on_i_sep<-NULL
    same_hla<-NULL
    for(h in 1:length(l)){
     hla_own<-strsplit(index[l[h],4], " ")[[1]]
     count<-0
     count_sep<-0
     for(unq in unique(peptide[,21])){
      hit<-which(!is.na(match(peptide[,21], unq)))
      if(length(hit)==1){
       print(index[i,1])
       flag<-TRUE
       break
      }
      peptide_hit<-peptide[hit[sort(match(gsub("\\*","_",gsub(":", "", hla_own)), peptide[hit,1]))],]
      if(is.null(nrow(peptide_hit))) peptide_hit<-t(peptide_hit)      
      if(nrow(peptide_hit)==0) next
      if(min(as.numeric(peptide_hit[,5])) < ic50) {
       count <- count + 1.0 / length(unique(peptide[,21]))
      }
      count_sep<-count_sep + 
           (length(which(as.numeric(peptide_hit[,5]) <ic50)) / nrow(peptide_hit)) / length(unique(peptide[,21]))
     }
     if(flag)break
     neoAntigen_on_i<-c(neoAntigen_on_i, count)
     neoAntigen_on_i_sep<-c(neoAntigen_on_i_sep, count_sep)
     same_hla<-c(same_hla, length(which(!is.na(match(hla_own, strsplit(hla, " ")[[1]][-1])))))
    }
    if(flag)break
    if(length(FULL) < len){FULL<-c(FULL, list(NULL))}
    FULL[[len]]<-rbind(FULL[[len]], neoAntigen_on_i)
    rownames(FULL[[len]])[nrow(FULL[[len]])]<-hla

    if(length(FULL_sep) < len){FULL_sep<-c(FULL_sep, list(NULL))}
    FULL_sep[[len]]<-rbind(FULL_sep[[len]], neoAntigen_on_i_sep)
    rownames(FULL_sep[[len]])[nrow(FULL_sep[[len]])]<-hla

    if(length(HIT) < len){HIT<-c(HIT, list(NULL))}
    HIT[[len]]<-rbind(HIT[[len]], same_hla)
 ...
 for(j in 1:length(FULL)){
  ...
  x<-as.numeric(HIT[[j]])
  boxplot(sapply(0:2, function(y) val[grep(y, x)]), names=0:2, xlab="Num. Hit", 
  ...
  plot(0:2, sapply(0:2, function(z) median(FULL[[j]][1,grep(z, HIT[[j]][1,])])), 
   type="b", xlim=c(0,2), ylim=ylim_, col=count, pch=count)
  dist<-c(dist, lm(FULL[[j]][1, ] ~ HIT[[j]][1, ])$coefficients[2])
  abline(lm(FULL[[j]][1, ] ~ HIT[[j]][1, ]), col=count, pch=count, lty=3)
  for(k in 1:nrow(FULL[[j]])){
   count<-count+1
   par(new=TRUE)
   plot(0:2, sapply(0:2, function(z) median(FULL[[j]][k, grep(z, HIT[[j]][k,])])), 
    type="b", xlim=c(0, 2), ylim=ylim_, xlab="", ylab="", xaxt="n", yaxt="n", col=count, pch=count)
 ...
 for(j in 1:length(FULL_sep)){
  ...
  count<-1
  ylim_<-c(max(0, min(val) * 0.9), min(1, max(val)* 1.1))
  plot(0:2, sapply(0:2, function(z) median(FULL_sep[[j]][1,grep(z, HIT[[j]][1,])])), 
   type="b", xlim=c(0,2), ylim=ylim_, col=count, pch=count)
  ...
  for(k in 1:nrow(FULL_sep[[j]])){
   count<-count+1
   par(new=TRUE)
   plot(0:2, sapply(0:2, function(z) median(FULL_sep[[j]][k, grep(z, HIT[[j]][k,])])), 
    type="b", xlim=c(0, 2), ylim=ylim_, xlab="", ylab="", xaxt="n", yaxt="n", col=count, pch=count)
...
write.table(eachsubject,paste("StatSnvPersonMatrix/HLA.Boxplot.Class2.FULL.lm",u,fpkm,"txt",sep="."),
        row.names=FALSE, col.names=FALSE, quote=FALSE, sep="\t")
write.table(eachposition,paste("StatSnvPersonMatrix/HLA.Boxplot.Class2.FULL.position",u,fpkm,"txt",sep="."),
        row.names=FALSE, col.names=FALSE, quote=FALSE, sep="\t")
print("Remove")
print(remove)
dev.off()
